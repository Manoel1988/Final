...
CREATE POLICY "Enable write access for admins and owners"
...
USING (
  EXISTS (
    ...
    AND (
      ...
      OR auth.uid() IN (
        SELECT user_id FROM companies WHERE id = company_products.company_id
      )
    )
  )
);
...
-- Create function to get company product price history
CREATE OR REPLACE FUNCTION get_company_product_price_history(
  company_id uuid,
  product_id uuid
)
RETURNS jsonb
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'assignment', jsonb_build_object(
      'id', cp.id,
      'override_price', cp.price_override,
      'effective_price', COALESCE(cp.price_override, p.price),
  ...
  FROM company_products cp
  JOIN products p ON cp.product_id = p.id
  WHERE cp.company_id = get_company_product_price_history.company_id
    AND cp.product_id = get_company_product_price_history.product_id;
